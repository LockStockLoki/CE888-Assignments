...
%Balance between first, exploitation and second, exploration term.
%As each node is visted the denominator of the exploration term is increased. Decreasing the contribution 
%However if another child node is visted the numerator increases, increasing the exploration values of other child nodes
%Exploration term ensures a zero-probability that a child node will fail to be explored on an appropriately long time-line

%Each node has two values assigned to it, the number of visits $N_{(v)}$ and $Q_{(v)}$ is the reward of all playouts that passed though this node. Therefore $\frac{Q_{v}}{N_{v}}$ is an approximation of the nodes value.
